...
public class FragmentSettings extends PreferenceFragmentCompat {
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        File tsd = new File(Environment.getExternalStorageDirectory() + "/WalkingLogger");
        boolean isGPSLoggerFolder = true;
        if (!tsd.exists()) {
            isGPSLoggerFolder = tsd.mkdir();
        }
        tsd = new File(Environment.getExternalStorageDirectory() + "/WalkingLogger/AppData");
        if (!tsd.exists()) {
            isGPSLoggerFolder = tsd.mkdir();
        }
        Log.w("myApp", "[#] FragmentSettings.java - " + (isGPSLoggerFolder ? "Folder /WalkingLogger/AppData OK" : "Unable to create folder /WalkingLogger/AppData"));
        ...
        File sd_old = new File(Environment.getExternalStorageDirectory() + "/WalkingLogger/AppData/WW15MGH.DAC");
